...
3. **Judge** — `Check.Runner` folds pending migrations through the 11 checks, threading a catalog overlay (`Catalog.apply_migration/2`) so migration N sees the world 1..N−1 created; within one migration, `Check.Helpers.fold_operations/3` threads per-op. Each rule derives `DDL.Effects` → `DDL.Locks` (a literal `(class, engine, version) → {lock, cost}` data table) and combines with catalog scale/traffic in `Severity.assess/6`.
